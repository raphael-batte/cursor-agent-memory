...
def enqueue(
    ...
    *,
    ...
    items = [i for i in items if i.get("chat_id") != chat_id]
    items.append(
        {
            "chat_id": chat_id,
...
def mark_done(memory_home: Path, chat_id: str) -> bool:
    items = _load(memory_home)
...
def session_start_user_message(
    ...
    pending = list_pending(memory_home, workspace_slugs=slugs if slugs else None)
    if not pending:
        return None
    lines = [
